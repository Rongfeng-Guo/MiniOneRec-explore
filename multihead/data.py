...
def split_sid_tokens_from_string(sid_str):
    """
    把拼接后的 SID 字符串拆成 3 个 token。
    例如: "<a_10><b_20><c_5>" -> ["<a_10>", "<b_20>", "<c_5>"]
    如果你的 SID 不是这种格式，需要按实际格式改这里。
    """
    sid_str = str(sid_str).strip()
    parts = re.findall(r"<[^<>]+>", sid_str)
    if len(parts) != 3:
        raise ValueError(f"Expect 3 SID tokens, got {len(parts)} from: {sid_str}")
    return parts


def build_sid_token_to_level_id(index_path):
    """
    从 index.json 中构建每一层 token -> 类别id 的映射。
    返回:
        token_to_id_per_level: [dict_level0, dict_level1, dict_level2]
        num_classes_per_level: [K0, K1, K2]
    """
    ...
    token_to_id_per_level = []
    num_classes_per_level = []
    for lv in range(3):
        vocab = sorted(list(level_sets[lv]))
        token_to_id = {tok: i for i, tok in enumerate(vocab)}
        token_to_id_per_level.append(token_to_id)
        num_classes_per_level.append(len(vocab))

    return token_to_id_per_level, num_classes_per_level
...
class Tokenizer:
    ...
    def encode(self, s: str, bos: bool, eos: bool) -> List[int]:
        ...
        while len(t) > 0 and self.bos_id is not None and t[0] == self.bos_id:
            t = t[1:]
        while len(t) > 0 and self.eos_id is not None and t[-1] == self.eos_id:
            t = t[:-1]
...
class SidSFTDataset(CSVBaseDataset):
    def __init__(
        ...
        self.sid_token_to_id = None
        self.sid_num_classes = None

        if sid_index_path is not None:
            self.sid_token_to_id, self.sid_num_classes = build_sid_token_to_level_id(sid_index_path)

        self.get_inputs()

    def get_target_sid_level_ids(self, sid_str):
        if self.sid_token_to_id is None:
            raise ValueError("sid_index_path is required for level-aware supervision.")

        sid_tokens = split_sid_tokens_from_string(sid_str)
        l0 = self.sid_token_to_id[0][sid_tokens[0]]
        l1 = self.sid_token_to_id[1][sid_tokens[1]]
        l2 = self.sid_token_to_id[2][sid_tokens[2]]
        return l0, l1, l2
...
"""
        ...
        full_tokens = tokens + golden_tokens
        full_attention_mask = [1] * len(full_tokens)
        full_labels = [-100] * input_prompt_len + full_tokens[input_prompt_len:]

        if len(full_tokens) >= self.max_len:
            print(len(full_tokens))

        input_ids = full_tokens[-self.max_len:]
        attention_mask = full_attention_mask[-self.max_len:]
        labels = full_labels[-self.max_len:]

        raw_level_pos = input_prompt_len - 1
        left_cut = max(0, len(full_tokens) - self.max_len)
        level_pos = raw_level_pos - left_cut

        if level_pos < 0:
            level_pos = 0
        if level_pos >= len(input_ids):
            level_pos = len(input_ids) - 1

        target_sid_str = str(self.data.iloc[idx]["item_sid"]).strip()
        target_sid_l0, target_sid_l1, target_sid_l2 = self.get_target_sid_level_ids(target_sid_str)

        return {
            "input_ids": input_ids,
            "attention_mask": attention_mask,
            "labels": labels,
            "target_sid_l0": target_sid_l0,
            "target_sid_l1": target_sid_l1,
            "target_sid_l2": target_sid_l2,
            "level_pos": level_pos,
        }
