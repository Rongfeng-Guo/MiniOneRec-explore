...
def normalize_sid_text(x: str) -> str:
    return str(x).strip().strip('"').strip()


def safe_split_sid(x: str):
    x = normalize_sid_text(x)
    try:
        return split_sid_tokens_from_string(x)
    except Exception:
        return None
...
def hierarchical_match_score(
    ...
    pred_tokens = safe_split_sid(pred_sid)
    target_tokens = safe_split_sid(target_sid)

    if pred_tokens is None or target_tokens is None:
        return 0.0
    if len(pred_tokens) < 3 or len(target_tokens) < 3:
        return 0.0

    score = 0.0
    if pred_tokens[0] == target_tokens[0]:
        score += w_l0
    if pred_tokens[1] == target_tokens[1]:
        score += w_l1
    if pred_tokens[2] == target_tokens[2]:
        score += w_l2
...
def train(
    ...
    print(f"[RL] resume_from_checkpoint = {resume_from_checkpoint!r}")
    print(f"[RL] train_batch_size = {train_batch_size}")
    print(f"[RL] eval_batch_size = {eval_batch_size}")
    print(f"[RL] num_generations = {num_generations}")
    print(f"[RL] beam_search = {beam_search}")
    ...
    # 你这个项目里，24 + beam_search + num_generations=16 已经实锤会触发 batch mismatch
    if beam_search and num_generations == 16 and train_batch_size != 32:
        raise ValueError(
            f"[RL] Invalid config detected: beam_search={beam_search}, "
            f"num_generations={num_generations}, train_batch_size={train_batch_size}. "
            f"Use train_batch_size=32 to avoid prompt/completion batch mismatch."
        )
    if beam_search and num_generations == 16 and eval_batch_size != 64:
        raise ValueError(
            f"[RL] Invalid config detected: beam_search={beam_search}, "
            f"num_generations={num_generations}, eval_batch_size={eval_batch_size}. "
            f"Use eval_batch_size=64 for the validated config."
        )

    if resume_from_checkpoint:
        print(
            f"[RL] resume_from_checkpoint was provided: {resume_from_checkpoint}, "
            f"but optimizer/scheduler state resume is disabled in this script to avoid OOM. "
            f"Training will continue from model_path initialization only."
        )
    ...
    def ndcg_rule_reward(prompts, completions):
        ...
        for i, completion in enumerate(completions):
            ...
            if pred == tgt:
                ...
                group_rewards.append(ndcg_rewards[i % num_generations])
    ...
    def hierarchical_ndcg_reward(prompts, completions):
        ...
        for i, completion in enumerate(completions):
            ...
            rank_bonus = -ndcg_rewards[i % num_generations]
    ...
    try:
        ...
        print("[RL] Exception occurred during training.")
        print("[RL] " + repr(e))
        print(traceback.format_exc())

        # 最佳努力保存当前模型，避免中途报错完全白跑
        if is_main_process():
            try:
                crash_dir = os.path.join(output_dir, "crash_checkpoint")
                os.makedirs(crash_dir, exist_ok=True)
                trainer.save_model(crash_dir)
                trainer.model.save_pretrained(crash_dir)
                tokenizer.save_pretrained(crash_dir)

                with open(os.path.join(crash_dir, "crash_info.txt"), "w", encoding="utf-8") as f:
                    f.write(repr(e))
                    f.write("\n\n")
                    f.write(traceback.format_exc())

                print(f"[RL] Emergency checkpoint saved to: {crash_dir}")
            except Exception as save_e:
                print(f"[RL] Failed to save emergency checkpoint: {repr(save_e)}")
